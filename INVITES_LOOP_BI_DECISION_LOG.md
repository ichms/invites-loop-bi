...
| D-08 | **SCD Type 1 dims only. No Type 2.** A current-state attribute may live in a dim only when labelled as current; historical/as-of attributes (device assignment, site, cohort status) need event-time facts or a tested temporal bridge | SCD2 done wrong produces confidently wrong history and will be done wrong by someone learning it from a blog post. |
...
| `fct_wearable_*` (6) | one source-shaped observation | Discovery lifelog wearable children | Point, interval and session grains remain separate (D-31); daily reporting stays in `fct_wearable_day`. |
...
| D-31 | **Raw wearable observations stay in six source-shaped facts, separate from `fct_measurement` and `fct_wearable_day`** | Step and heart-rate are point samples, activity and sleep stage are intervals, and sleep is a session; forcing them into one nullable relation gives no honest shared grain. The six `fct_wearable_*` facts preserve those grains, while `fct_wearable_day` remains the reporting aggregate. Exact duplicate payloads collapse to one stable observation and retain multiplicity in `source_row_count`, so transport duplicates do not masquerade as distinct measurements and no delivered-row count is lost. |
| D-32 | **Current deployment site is the one active approved Ulsan/Jeju customer link; it is not historical attribution** | Owner allow-list: Ulsan `2e0a3387-7058-4f9e-a134-2017f7b7000b`, Jeju `778d4ff7-ab76-4070-a9a9-716fac93d9c9`. `auth_customer` also contains app tenants, so IDs are explicit. `dim_user.site_id` may support a current-site filter, but known in-place flips erase prior value and switch time; as-of metrics still require an upstream write contract and temporal bridge. |

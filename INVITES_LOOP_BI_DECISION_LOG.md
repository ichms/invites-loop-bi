...
| **Apache Superset** | Four moving parts — Redis, Celery workers, metadata DB, `superset_config.py`. Failure mode for an unattended install is not "insufficiently powerful," it is "the Celery worker died in November and alerts silently stopped." Chart-building UX is also worse for casual users. |
...
| **Lightdash** | Requires a dbt project as an absolute dependency (none exists today), and — decisively — metric changes become a git PR, which no Planning Team member will open. Best governance fit of the four, wrong constraint. |
